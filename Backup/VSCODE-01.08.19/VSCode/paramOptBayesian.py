...
def hyperopt(param_space,num_eval):
    ...
    def objective_function(params):
        scheduler = Scheduler(layoutFile='outputDM.csv', agvFile='agvs2.xlsx', \
                      requestFile='transportOrders3.xlsx', \
                    stationFile='stations.xlsx',hyperparams=params)

        solution =scheduler.solve(15)
        bestScore= solution.get('bestScores')[-1][1]
        return {'loss':bestScore, 'status':STATUS_OK, 'solution':solution, 'paramSet':scheduler.hyperparams}
    ...
    history=[(x['result']['paramSet'], x['result']['solution'], x['result']['loss']) for x in trials.trials]
    
    
    print('optimization completed / Running experiment with best parameters')
    scheduler = Scheduler(layoutFile='outputDM.csv', agvFile='agvs2.xlsx', \
                      requestFile='transportOrders3.xlsx', \
                    stationFile='stations.xlsx',hyperparams=best_param)
...
bestParam, loss, solution, history = hyperopt(Hyperparams,1000)

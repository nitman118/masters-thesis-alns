...
#         obj_unloaded = grb.quicksum(0 if REQID[v][r]<998 else 1*(D[r,v] - S[r,v]) for r in range(REQ[v]) for v in range(numAGVs)) 
        ...
        for v1 in range(numAGVs):
            m.addConstr(B[0,v1]==100, name=f"B_0_init_{v1}") #remove hardcoded 100 here
...
#KPI and other functionalities
    ...
    def getGreedyTaskListScore(self, alpha=0.8):
        tardinessTime=0
        totalUnloadedTime=0
        ...
        return alpha*tardinessTime+(1-alpha)*totalUnloadedTime
    ...
    def getScheduleKPI(self, taskSchedule=None):
        ...
        return tardiness,totalTravel,ult,totalTravelTime,totalTravelTimeCost
    
    def getScoreALNS(self,taskSchedule=None, alpha=0.8):
        '''
        returns score of a schedule, used in alns algorithm
        '''
        tardiness,_,_,_,totalTravelCost = self.getScheduleKPI(taskSchedule)
        return (alpha*tardiness + (1-alpha)*(totalTravelCost))
